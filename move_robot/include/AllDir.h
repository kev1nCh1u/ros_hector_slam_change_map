...
bool alldir::qrcodemove()
{
	...
	{
			trans_angle = qrcode_A - 90;
			if(trans_angle < 0)trans_angle = trans_angle + 360;
	}
	else if(qrcode_direction == 2)
	{
			trans_angle = qrcode_A + 180;
			if(trans_angle >= 360)trans_angle = trans_angle - 360;
	}
		          ...
		          if(fabs(qrcode_y) <= 1)
						...
						switch(angle_mode)
						{
								...
								case 1:
									...
									break;
								case 2:
									if((qrcode_th>-1.58 && qrcode_th <= -1.57) || (qrcode_th<-1.56 && qrcode_th >= -1.57))
									{
									...
									break;
								case 3:
									if((qrcode_th>1.56 && qrcode_th <= 1.57) || (qrcode_th<1.58 && qrcode_th>=1.57))
									{
		...
		if(isInitial){
			...
			//讓所有輪子歸0
				...
				if(kinematics_model == "omni" || kinematics_model == "omni_2"){
					...
					omni_first_angle = goal_angle - robot_pos.z();
					return true;
				}
    ...
    /////////////////////////
		...
		if(now_index < First_tracking ){
			...
			else if(kinematics_model == "omni")
			{
					...
					float now_angular_error = finial_target_pos.z() - robot_pos.z();
					angular_velocity_error = omni_first_angle - now_angular_error;
